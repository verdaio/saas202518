...
```bash
# Database
DATABASE_URL=postgresql://postgres:postgres@localhost:5418/saas202518

# Redis
REDIS_URL=redis://localhost:6418
...
**Navigate to:**
```
https://github.com/YOUR_ORG/saas202518/settings/secrets/actions
```
...
| `AZURE_REGISTRY_NAME` | Container Registry name | `saas202518acr` |
...
az containerapp restart --name saas202518-staging --resource-group saas202518-rg
az containerapp restart --name saas202518-prod --resource-group saas202518-rg
```
...
az ad sp create-for-rbac \
  --name "saas202518-github-actions-new" \
  --role contributor \
  --scopes /subscriptions/{sub-id}/resourceGroups/saas202518-rg \
...
1. Verify secret exists:
   ```bash
   az keyvault secret show --vault-name saas202518-kv --name SECRET-NAME
   ```
...
2. Check access policy:
   ```bash
   az keyvault show --name saas202518-kv --query properties.accessPolicies
   ```
...
az keyvault secret set --vault-name saas202518-kv --name SECRET-NAME --value "value"

# Get secret from Key Vault
az keyvault secret show --vault-name saas202518-kv --name SECRET-NAME --query value -o tsv

# List all secrets
az keyvault secret list --vault-name saas202518-kv

# Delete secret
az keyvault secret delete --vault-name saas202518-kv --name SECRET-NAME
